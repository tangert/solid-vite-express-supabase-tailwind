...
var [major, minor] = process.version.slice(1).split(".").map((x) => Number(x));
var loaderAvailable = (major > 16 || major === 16 && minor >= 12) && major < 20;
var cli = cac("vavite");
    ...
    },
    {
      ...
      onStartBuildStep(info) {
        initialConfig.logger.info(
          (info.currentStepIndex ? "\n" : "") + pico.cyan("vavite: " + version) + (info.currentStep.description ? pico.white(" " + info.currentStep.description) : pico.white(" running build step") + " " + pico.blue(info.currentStep.name)) + " (" + pico.green(
...
cli.command("serve [root]", "Start a dev server").alias("dev").option("--host [host]", `[string] specify hostname`).option("--port <port>", `[number] specify port`).option("--https", `[boolean] use TLS + HTTP/2`).option("--open [path]", `[boolean | string] open browser on startup`).option("--cors", `[boolean] enable CORS`).option("--strictPort", `[boolean] exit if specified port is already in use`).option(
  "--force",
  `[boolean] force the optimizer to ignore the cache and re-bundle`
).option("--use-loader", `[boolean] use ESM loader (experimental)`).action(
  async (root, options) => {
    if (options.useLoader) {
      if (!loaderAvailable) {
        ...
        const options2 = (process.env.NODE_OPTIONS ? process.env.NODE_OPTIONS + " " : "") + "-r vavite/suppress-loader-warnings --loader vavite/node-loader";
        const cp = spawn(process.execPath, process.argv.slice(1), {
          stdio: "inherit",
          env: {
            ...process.env,
            NODE_OPTIONS: options2
          }
        });
        cp.on("error", (err) => {
          console.error(err);
    ...
    try {
      ...
      info(
        `
  ${pico.green(
          pico.cyan("vavite: " + version + " (vite: " + viteVersion + ")") + " development server is running"
